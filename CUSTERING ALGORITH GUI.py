...
0 TO EXIT AND SAVE:\n \n""")  # saving inputs in text file
    ...
    def user_input(self, file_data):
        ...
        length, breadth = x_axisx, y_axisx
    ...
    def cluster_area_wise(self, length, breadth, nodes, file_data):
        ...
        number_of_clusters = cluster_getx  # can be changed to user input later on

        print("ENTER THE NUMBER OF CLUSTERS YOU WANT TO CREATE :"+str(number_of_clusters))

        file_data.writelines("ENTER THE NUMBER OF CLUSTERS YOU WANT TO CREATE :"+str(number_of_clusters)+"\n")
    ...
    def sergation_of_nodes_in_cluster(self, length, breadth, nodes, cluster_list, particles, file_data):
        ...
        for each_partilcle in particles:
            for node_cluster in cluster_list:
                if ((each_partilcle[1][0] >= node_cluster[2][0][0] and each_partilcle[1][0] <
                     node_cluster[2][0][1]) and (
                        each_partilcle[1][1] >= node_cluster[2][1][0] and each_partilcle[1][1] <
                        node_cluster[2][1][1])):
    ...
    def random_deployment(self, length, breadth, nodes, cluster_list, file_data):
        ...
        for i in range(no_of_particles):
            ...
            x_cordinate = round(random.randrange(0, length), 2) + round(random.random(),
                                                                        2)  # creates the x cordinate of the particle in the range 0-100 units
            y_cordinate = round(random.randrange(0, breadth), 2) + round(random.random(),
                                                                         2)  # creates the y cordinate of the particles in the range 0-100 units
    ...
    def analyze(self, length, breadth, cluster_list, particles, file_data):
        ...
        self.ACO_fine_tuning(leader_coordinates, leader_node, file_data, breadth, length, cluster_list,
                             particles)
    ...
    def ACO_fine_tuning(self, leader_coordinates, leader_node, file_data, breadth, length, cluster_list,
                        particles):
        ...
        for i in range(0, len(leader_coordinates)):
            ...
            for j in range(0, len(leader_coordinates)):
                if leader_coordinates[i][0] == leader_coordinates[j][0]:
                    ...
                    distance = (((leader_coordinates[i][1] - leader_coordinates[j][1]) ** 2) + (
                            (leader_coordinates[i][2] - leader_coordinates[j][2]) ** 2)) ** 0.5
        ...
        nx.draw_networkx(H, pox, node_size=600, node_color='red', edges=edges, edge_color=colors,
                         width=weights)
...
class CLUSTRING_NETWORK_GUI():
    ...
    def __init__(self, win):
        ...
        self.type_of_cluster_creation=["AREA WISE","DEFAULT"]
    ...
    def Start_Clustering(self):
        global x_axisx
        x_axisx=int(self.x_axis.get())
        global y_axisx
        y_axisx=int(self.y_axis.get())
        global nodesx
        nodesx=int(self.nodes.get())
        global cluster_getx
        cluster_getx=int(self.cluster.get())
        global cluster_typex
        cluster_typex=str(self.cluster_type.get())
        global deployment_typex
        deployment_typex=str(self.deployment_type.get())
        global intervalx
        intervalx=int(self.interval.get())
